...
public class GlobalExceptionHandler {
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        if (message.contains("wms_")) {
            log.error("[WMS 仓库管理系统 laby-module-wms - 表结构未导入][参考 https://doc.iocoder.cn/wms/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[WMS 仓库管理系统 laby-module-wms - 表结构未导入][参考 https://doc.iocoder.cn/wms/build/ 开启]");
        }
        // 7. CRM 系统
        ...
        if (message.contains("mes_")) {
            log.error("[MES 系统 laby-module-mes - 表结构未导入][参考 https://doc.iocoder.cn/mes/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[MES 系统 laby-module-mes - 表结构未导入][参考 https://doc.iocoder.cn/mes/build/ 开启]");
        }
        // 9. IM 即时通讯
        if (message.contains("im_")) {
            log.error("[IM 即时通讯 laby-module-im - 表结构未导入][参考 https://doc.iocoder.cn/im/build/ 开启]");
            return CommonResult.error(NOT_IMPLEMENTED.getCode(),
                    "[IM 即时通讯 laby-module-im - 表结构未导入][参考 https://doc.iocoder.cn/im/build/ 开启]");
        }
        // 10. 支付平台
