...
import com.laby.module.ai.controller.admin.knowledge.vo.document.*;
...
public class AiKnowledgeDocumentController {
    ...
    @PostMapping("/retry-ingest")
    @Operation(summary = "重新入库（分段+向量化）")
    @PreAuthorize("@ss.hasPermission('ai:knowledge:update')")
    public CommonResult<Boolean> retryKnowledgeDocumentIngest(@RequestParam("id") Long id) {
        documentService.retryKnowledgeDocumentIngest(id);
        return success(true);
    }
