package com.laby.module.bpm.framework.flowable.core.util;
...
import com.laby.framework.common.util.number.NumberUtils;
import com.laby.framework.common.util.string.StrUtils;
import com.laby.module.bpm.controller.admin.definition.vo.model.simple.BpmSimpleModelNodeVO;
import com.laby.module.bpm.controller.admin.task.vo.task.BpmTaskRespVO;
import com.laby.module.bpm.enums.definition.*;
import com.laby.module.bpm.framework.flowable.core.enums.BpmnModelConstants;
...
import static com.laby.module.bpm.framework.flowable.core.enums.BpmnModelConstants.*;
...
public class BpmnModelUtils {
     ...
     */
    /**
     * 根据流程变量解析排他网关实际走出的连线（用于流程图高亮）
     */
    public static SequenceFlow resolveExclusiveGatewayOutgoingFlow(Gateway gateway, Map<String, Object> variables) {
        return findMatchSequenceFlowByExclusiveGateway(gateway, variables != null ? variables : Map.of());
    }
    ...
    /**
     * 根据历史活动顺序，补全网关分支等未写入历史的连线/节点高亮（与右侧审批时间线对齐）
     */
    public static void supplementProcessHighlight(BpmnModel bpmnModel,
        ...
        if (bpmnModel == null || CollUtil.isEmpty(activities)) {
            return;
        }
        Map<String, Object> variables = processVariables != null ? processVariables : Map.of();
        // 开始节点：Flowable 常不记录 start→首节点的 sequenceFlow
        activities.stream()
                .filter(activity -> StrUtil.equals(activity.getActivityType(), BpmnXMLConstants.ELEMENT_EVENT_START)
                        && activity.getEndTime() != null)
                .map(HistoricActivityInstance::getActivityId)
                .forEach(finishedTaskActivityIds::add);
        List<HistoricActivityInstance> ordered = activities.stream()
                .filter(activity -> activity.getStartTime() != null && StrUtil.isNotEmpty(activity.getActivityId()))
                .filter(activity -> !StrUtil.equals(activity.getActivityType(), BpmnXMLConstants.ELEMENT_SEQUENCE_FLOW))
                .sorted(Comparator.comparing(HistoricActivityInstance::getStartTime))
                .toList();
        Set<String> historicActivityIds = CollectionUtils.convertSet(activities, HistoricActivityInstance::getActivityId);
        for (int i = 0; i < ordered.size() - 1; i++) {
            supplementPathBetween(bpmnModel,
                    ordered.get(i).getActivityId(),
                    ordered.get(i + 1).getActivityId(),
                    historicActivityIds,
                    variables,
                    finishedTaskActivityIds,
                    finishedSequenceFlowActivityIds);
        }
        if (CollUtil.isNotEmpty(unfinishedTaskActivityIds)) {
            unfinishedTaskActivityIds.forEach(activityId ->
                    finishedTaskActivityIds.remove(activityId));
        }
    }

    private static void supplementPathBetween(BpmnModel bpmnModel, String fromId, String toId,
                                              Set<String> historicActivityIds,
                                              Map<String, Object> processVariables,
                                              Set<String> finishedTaskActivityIds,
                                              Set<String> finishedSequenceFlowActivityIds) {
        if (StrUtil.equals(fromId, toId)) {
            return;
        }
        FlowElement from = getFlowElementById(bpmnModel, fromId);
        FlowElement to = getFlowElementById(bpmnModel, toId);
        if (from == null || to == null) {
            return;
        }
        // 排他网关：按流程变量（needSecondRound、riskHighCount 等）高亮实际分支，避免误标「跳过二轮」
        if (from instanceof Gateway gateway) {
            SequenceFlow matchedFlow = resolveExclusiveGatewayOutgoingFlow(gateway, processVariables);
            if (matchedFlow != null) {
                finishedSequenceFlowActivityIds.add(matchedFlow.getId());
                finishedTaskActivityIds.add(fromId);
                FlowElement target = matchedFlow.getTargetFlowElement();
                if (target != null && !StrUtil.equals(target.getId(), toId)) {
                    supplementPathBetween(bpmnModel, target.getId(), toId, historicActivityIds, processVariables,
                            finishedTaskActivityIds, finishedSequenceFlowActivityIds);
                }
                return;
            }
        }
        Deque<PathState> queue = new ArrayDeque<>();
        queue.add(new PathState(from, new ArrayList<>(), new ArrayList<>()));
        Set<String> visitedElements = new HashSet<>();
        visitedElements.add(fromId);
        while (!queue.isEmpty()) {
            PathState state = queue.poll();
            if (StrUtil.equals(state.element.getId(), toId)) {
                state.flowIds.forEach(finishedSequenceFlowActivityIds::add);
                state.elementIds.forEach(finishedTaskActivityIds::add);
                return;
            }
            for (SequenceFlow sequenceFlow : getElementOutgoingFlows(state.element)) {
                if (state.flowIds.contains(sequenceFlow.getId())) {
                    continue;
                }
                FlowElement target = sequenceFlow.getTargetFlowElement();
                if (target == null || visitedElements.contains(target.getId())) {
                    continue;
                }
                // 仅沿历史出现过的节点寻路，避免「跳过二轮」时误高亮二轮分支
                if (!StrUtil.equals(target.getId(), toId) && !historicActivityIds.contains(target.getId())) {
                    continue;
                }
                List<String> nextFlows = new ArrayList<>(state.flowIds);
                nextFlows.add(sequenceFlow.getId());
                List<String> nextElements = new ArrayList<>(state.elementIds);
                if (!(target instanceof EndEvent)) {
                    nextElements.add(target.getId());
                }
                visitedElements.add(target.getId());
                queue.add(new PathState(target, nextFlows, nextElements));
            }
        }
    }

    private record PathState(FlowElement element, List<String> flowIds, List<String> elementIds) {
    }
