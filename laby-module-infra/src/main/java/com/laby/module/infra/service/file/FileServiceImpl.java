...
import com.laby.module.infra.framework.file.core.utils.FilePathUtils;
...
public class FileServiceImpl implements FileService {
    ...
    private static final String INTERNAL_FILE_URL_MARKER = "/admin-api/infra/file/";
    ...
    public String createFile(byte[] content, String name, String directory, String type) {
        return createFileAndReturn(content, name, directory, type).getUrl();
    }

    @Override
    @SneakyThrows
    public Long createFileReturnId(byte[] content, String name, String directory, String type) {
        return createFileAndReturn(content, name, directory, type).getId();
    }
     ...
     */
    @SneakyThrows
    private FileDO createFileAndReturn(byte[] content, String name, String directory, String type) {
    ...
    public byte[] getFileContentByUrl(String url) throws Exception {
        ...
        // 1. 本系统文件 URL：直接从存储读取，避免相同 url 重复入库导致 selectOne 失败
        byte[] content = getFileContentFromInternalUrl(cleanUrl);
        if (content != null) {
            return content;
        }
        if (!StrUtil.equals(cleanUrl, url)) {
            content = getFileContentFromInternalUrl(url);
            if (content != null) {
                return content;
            }
        }

        // 2. 通过 url 查库（取最新一条，兼容重复上传）
        FileDO file = getLatestFileByUrl(cleanUrl);
        if (file == null && !StrUtil.equals(cleanUrl, url)) {
            file = getLatestFileByUrl(url);
        }
        if (file != null) {
            log.debug("[getFileContentByUrl][从文件存储读取 url({}) path({})]", url, file.getPath());
            return getFileContent(file.getConfigId(), file.getPath());
        }

        // 3. HTTP 下载外链
        log.debug("[getFileContentByUrl][HTTP 下载外链 url({})]", url);
        return HttpRequest.get(url).timeout(120_000).execute().bodyBytes();
    }

    /**
     * 解析本系统文件访问 URL，格式：{domain}/admin-api/infra/file/{configId}/get/{path}
     */
    private byte[] getFileContentFromInternalUrl(String url) throws Exception {
        int markerIndex = url.indexOf(INTERNAL_FILE_URL_MARKER);
        if (markerIndex < 0) {
            return null;
        }
        String rest = url.substring(markerIndex + INTERNAL_FILE_URL_MARKER.length());
        int getIndex = rest.indexOf("/get/");
        if (getIndex <= 0) {
            return null;
        }
        Long configId = Long.valueOf(rest.substring(0, getIndex));
        String path = URLUtil.decode(rest.substring(getIndex + "/get/".length()), StandardCharsets.UTF_8, false);
        log.debug("[getFileContentByUrl][解析本系统 URL configId({}) path({})]", configId, path);
        return getFileContent(configId, path);
    }

    private FileDO getLatestFileByUrl(String url) {
        List<FileDO> list = fileMapper.selectList(new LambdaQueryWrapper<FileDO>()
