...
/**
 * 直接采纳一条待处理审核意见（用户对话指令即授权，无需二次弹框）。
 */
@Component("legal_adopt_opinion")
public class LegalAdoptOpinionTool {
    ...
    @Tool(name = "legal_adopt_opinion", description = "直接采纳一条待处理的审核意见")
    public Response adoptOpinion(
    ...
    private Response doAdopt(Request request, LegalAgentToolRuntimeContext toolContext) {
        ...
        response.setTitle(opinion.getTitle());
