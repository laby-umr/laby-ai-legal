...
import type { RecallDiagnostics } from '#/api/ai/knowledge/segment';
...
export namespace AiChatMessageApi {
  export interface ChatMessage {
    ...
    recallDiagnostics?: ChatRecallDiagnostics; // 召回诊断
  ...
  /** 单知识库召回诊断（对话合并项） */
  export interface ChatRecallDiagnosticsItem extends RecallDiagnostics {
    knowledgeId?: number;
  }

  /** 对话召回诊断（多知识库合并） */
  export interface ChatRecallDiagnostics {
    totalHitCount?: number;
    noAnswerGuard?: boolean;
    items?: ChatRecallDiagnosticsItem[];
  }
}
