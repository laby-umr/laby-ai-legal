...
/** 法务合同等场景：AI 润色角色设定 */
export function polishChatRoleSystemMessage(data: {
  draft: string;
  scene?: string;
  modelId?: number;
  existingSystemMessage?: string;
}) {
  return requestClient.post<{ systemMessage: string }>(
    '/ai/chat-role/polish',
    data,
    { timeout: 120_000 },
  );
...
/** 法务合同审核角色分类（与后端 AiChatRoleCategoryEnum.LEGAL_CONTRACT 一致） */
export const LEGAL_CONTRACT_CHAT_ROLE_CATEGORY = '法务合同';

/** 后台菜单注册的聊天角色页（AI 大模型 → 控制台 → 聊天角色） */
export const AI_CHAT_ROLE_MENU_PATH = '/ai/console/chat-role';
