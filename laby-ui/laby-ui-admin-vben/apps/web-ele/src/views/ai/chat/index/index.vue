...
import { getChatRole } from '#/api/ai/model/chatRole';
import {
...
import RolePromptMessage from './modules/message/role-prompt-message.vue';
import LegalOrchestrationProposalCard from './modules/message/legal-orchestration-proposal-card.vue';
...
const orchestrationProposalRefreshKey = ref(0);
const roleHasKnowledge = ref(false);
// 接收 Stream 消息
...
/** 刷新当前角色是否绑定知识库 */
async function refreshRoleKnowledge(roleId?: number | null) {
  if (!roleId) {
    roleHasKnowledge.value = false;
    return;
  }
  try {
    const role = await getChatRole(roleId);
    roleHasKnowledge.value = (role?.knowledgeIds?.length ?? 0) > 0;
  } catch {
    roleHasKnowledge.value = false;
  }
}
...
async function getConversation(id: null | number) {
  ...
  await refreshRoleKnowledge(conversation.roleId);
}
...
/**
 * 消息列表（不含虚拟 systemMessage，角色设定单独展示）
 */
const messageList = computed(() => activeMessageList.value);

/** 无真实消息时展示的角色设定卡片 */
const rolePromptContent = computed(() => {
  if (activeMessageList.value.length > 0) {
    return null;
  }
  const text = activeConversation.value?.systemMessage?.trim();
  return text || null;
});
...
/** 编排提案确认后，自动通知 Agent 继续（无需用户再打字） */
function buildOrchestrationContinuePrompt(action: string): string | null {
  switch (action) {
    case 'CLASSIFY_CONFIRM':
      return (
        '【系统通知】用户已在侧边栏确认执行「分类确认」。请直接继续下一步：' +
        '调用 legal_orchestration_propose_create_contracts 生成创建合同提案。' +
        '编排阶段不做 AI 审核；合同落库后系统自动解析并首轮 AI 审核。' +
        '无需再次口头向用户索要确认。'
      );
    case 'CREATE_TYPE_PACKAGE':
      return (
        '【系统通知】用户已在侧边栏确认执行「发布合同类型」。' +
        '请继续后续编排步骤（生成创建合同提案）。'
      );
    case 'CREATE_CONTRACTS_BATCH':
      return (
        '【系统通知】用户已在侧边栏确认执行「创建合同」。' +
        '请总结已创建的合同，并引导用户前往「我的合同」审阅页查看；系统会自动进行解析与首轮 AI 审核。'
      );
    default:
      return null;
  }
...
async function handleOrchestrationProposalExecuted(payload: {
  action: string;
  proposalNo: string;
}) {
  if (conversationInProgress.value) {
  ...
  const content = buildOrchestrationContinuePrompt(payload.action);
  if (!content) {
    return;
  }
  await doSendMessage(content);
  ...
  try {
    ...
    await sendChatMessageStream(
      ...
      async (res: any) => {
        ...
        // 如果内容、推理、知识库段落、召回诊断都为空，就不处理
        if (
          data.receive.content === '' &&
          !data.receive.reasoningContent &&
          !(data.receive.segments?.length) &&
          !data.receive.recallDiagnostics
        ) {
          return;
        }
        ...
        if (lastMessage) {
          if (data.receive.segments?.length) {
            lastMessage.segments = data.receive.segments;
          }
          if (data.receive.recallDiagnostics) {
            lastMessage.recallDiagnostics = data.receive.recallDiagnostics;
          }
          if (data.receive.webSearchPages?.length) {
            lastMessage.webSearchPages = data.receive.webSearchPages;
          }
        }
...
async function stopStream() {
  ...
  orchestrationProposalRefreshKey.value += 1;
}
...
<template>
  <Page
    auto-content-height
    content-class="flex min-h-0 flex-col overflow-hidden !p-4"
  >
    <div class="ai-chat-layout">
      ...
      <ElContainer direction="vertical" class="ai-chat-main">
        <ElHeader
        ...
        <ElMain class="relative m-0 min-h-0 flex-1 overflow-hidden p-0">
          <div
            class="absolute inset-0 m-0 flex flex-col p-0"
            :class="
          ...
          >
            ...
            <MessageList
              ...
              :conversation-in-progress="conversationInProgress"
              :role-has-knowledge="roleHasKnowledge"
        ...
        <ElFooter height="auto" class="flex shrink-0 flex-col !bg-card !p-0">
          <LegalOrchestrationProposalCard
            v-if="activeConversation?.roleId === LEGAL_ORCHESTRATION_ROLE_ID"
            class="mx-4 mt-1"
            :conversation-id="activeConversationId"
            :refresh-key="orchestrationProposalRefreshKey"
            @proposal-executed="handleOrchestrationProposalExecuted"
          />
          <form
            class="mx-4 mb-4 mt-1 flex flex-col rounded-xl border border-border p-2"
          >
            <textarea
...
.ai-chat-main :deep(.el-header) {
  border-radius: 12px 12px 0 0;
}

.ai-chat-main :deep(.el-footer) {
