...
import {
  formatKnowledgeHitCount,
  hasNoAnswerGuard,
} from '#/views/ai/chat/components/recall-diagnostics-utils';
...
export function useGridColumnsMessage(): VxeTableGridOptions['columns'] {
  return [
    ...
    },
    {
      ...
      minWidth: 90,
      formatter: ({ row }) =>
        formatKnowledgeHitCount(row as AiChatMessageApi.ChatMessage),
    },
    {
      title: '无引用守卫',
      minWidth: 100,
      formatter: ({ row }) =>
        hasNoAnswerGuard(row as AiChatMessageApi.ChatMessage) ? '是' : '-',
    },
    {
      field: 'createTime',
      title: '创建时间',
