...
export function useFormSchema(): VbenFormSchema[] {
  return [
    ...
    },
    {
      ...
      help: '推荐 0.60；超过 0.85 将显著降低召回率',
      rules: z
        .number()
        .min(0)
        .max(0.85, '相似度阈值不建议超过 0.85')
        .default(0.6),
      dependencies: {
        triggerFields: ['similarityThreshold'],
        trigger(values, formApi) {
          const threshold = values.similarityThreshold;
          if (typeof threshold === 'number' && threshold > 0.85) {
            formApi.setFieldError(
              'similarityThreshold',
              '阈值超过 0.85 将显著降低召回率，建议降至 0.60–0.75',
            );
          }
        },
      },
