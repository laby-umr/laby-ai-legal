...
/** 跳转到 RAG 黄金集测评 */
function handleRagEval(id: number) {
  router.push({
    name: 'AiKnowledgeRagEval',
    query: { id },
  });
}

/** 跳转到向量健康检查 */
function handleVectorHealth(id: number) {
  router.push({
        ...
        <TableAction
          :actions="[
            ...
            },
            {
              ...
              auth: ['ai:knowledge:query'],
              onClick: handleRagEval.bind(null, row.id),
            },
            {
              label: '向量检查',
              type: 'primary',
              ...
              icon: ACTION_ICON.EDIT,
              auth: ['ai:knowledge:update'],
              onClick: handleVectorHealth.bind(null, row.id),
            },
            {
