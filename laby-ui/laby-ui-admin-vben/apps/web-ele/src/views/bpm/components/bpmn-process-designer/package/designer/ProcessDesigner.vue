...
const createNewDiagram = async (xml: any) => {
  ...
  try {
    ...
    if (warnings?.length) {
      console.warn('[Process Designer] import warnings:', warnings);
    }
    await processReZoom();
  } catch (error: any) {
    const message = error?.message || String(error);
    console.error('[Process Designer] importXML failed:', error);
    ElMessage.error(`流程图加载失败：${message}`);
    throw error;
  }
};
...
const importLocalFile = () => {
  ...
  reader.addEventListener('load', async function () {
    const xmlStr = this.result as string;
    if (!xmlStr?.trim()) {
    ...
    try {
      ...
      emit('save', xmlStr);
      ElMessage.success('流程图已导入，可点击「适应画布」查看');
    } catch {
      // createNewDiagram 已提示错误
    } finally {
      refFile.value.value = '';
    }
  });
  reader.addEventListener('error', () => {
    ElMessage.error('读取 BPMN 文件失败');
  });
};
/* ------------------------------------------------ refs methods ------------------------------------------------------ */
