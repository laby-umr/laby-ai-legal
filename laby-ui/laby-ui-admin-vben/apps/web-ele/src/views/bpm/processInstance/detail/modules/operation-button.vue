...
async function openPopover(type: string) {
  ...
  // 关闭其它气泡，打开当前类型（原先仅处理已打开的项，导致首次点击「通过」弹窗不出现）
...
function getUpdatedProcessInstanceVariables() {
  ...
  // 业务表单：合并流程实例已有变量（如法务 needSecondRound、riskHighCount）
  if (props.processDefinition?.formType === BpmModelFormType.CUSTOM) {
    Object.assign(variables, props.processInstance?.formVariables || {});
  }
  return variables;
}
