...
const formData = ref<LegalAuditRuleApi.Rule>();
const getTitle = computed(() =>
  formData.value?.id
    ? $t('ui.actionTitle.edit', ['审核规则'])
    : $t('ui.actionTitle.create', ['审核规则']),
);
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onConfirm() {
    ...
    modalApi.lock();
    const data = (await formApi.getValues()) as LegalAuditRuleApi.Rule;
    try {
      await (formData.value?.id ? updateAuditRule(data) : createAuditRule(data));
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    const data = modalApi.getData<LegalAuditRuleApi.Rule>();
    if (!data?.id) {
      await formApi.resetForm();
      await formApi.setValues({ enabled: true, priority: 0 });
      return;
    }
    modalApi.lock();
    try {
      formData.value = await getAuditRule(data.id);
